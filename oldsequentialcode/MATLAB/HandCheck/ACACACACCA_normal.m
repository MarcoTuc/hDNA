...
nucleation = addreaction(fitmodel,'SS <-> NC'); %NOTE: it is a bimolecular reactions, it's just how the nucleation kinetic law is defined
nucleationlaw = addkineticlaw(nucleation,'nucleation');
zipping = addreaction(fitmodel,'NC <-> D');
zippinglaw = addkineticlaw(zipping,'zipping');

addparameter(fitmodel, 'k1', 4.22e6, 'units', '1/(molarity*second)');
addparameter(fitmodel, 'k2', 5e8, 'units', '1/second');
addparameter(fitmodel, 'e1', 1e2, 'units', '1/molarity');
addparameter(fitmodel, 'E', 1e6, 'units', '1/molarity');

set(nucleationlaw,'parametervariablenames',{'k1' 'e1'});
set(nucleationlaw,'SpeciesVariableNames',{'SS' 'NC'})
set(zippinglaw,'parametervariablenames',{'k2' 'e1' 'E'});
set(zippinglaw,'SpeciesVariableNames',{'NC' 'D'})

initial = 0.5e-4;
fitmodel.species(1).initialamount = initial;

conf = fitmodel.config;
set(conf, 'solvertype','ode15s');
set(conf.solveroptions, 'absolutetolerance',abstol);
set(conf.solveroptions, 'relativetolerance',reltol);
set(conf, 'timeunits',timeunits);
set(conf, 'stoptime',runtime);
sbioaccelerate(fitmodel);

fitsim = sbiosimulate(fitmodel);
sbioplot(fitsim);
hold on
sbioplot(sim_ACA);
...
paramstoestimate = {'k2','e1'};
estimate = estimatedInfo(paramstoestimate,'bounds',[]);
fitresult = sbiofit(fitmodel, G, responsemap, estimate, [], 'fminsearch',options,'UseParallel',true);
