...
reaction_a = addreaction(model, "SS1 + SS2 <-> L1");
...
reaction_d = addreaction(model, "SS1 + SS2 <-> D");
...
reaction_e = addreaction(model, "SS1 + SS2 <-> L2");
...
reaction_f = addreaction(model, "SS1 + SS2 <-> R1");
...
model.species(2).initialamount = pseudo0;
...
duplex = sim_ACA.Data(delay:end,5);
...
nucleation = addreaction(fitmodel,'SS <-> NC');
...
addparameter(fitmodel, 'kf1', 2e6, 'units', '1/second');
addparameter(fitmodel, 'kb1', 1, 'units', '1/second');
addparameter(fitmodel, 'kf2', 2e6, 'units', '1/second');
...
paramstoestimate = {'kf1','kf2'};
estimate = estimatedInfo(paramstoestimate,'bounds',[]);
fitresult = sbiofit(fitmodel, G, responsemap, estimate, [], 'nlinfit','UseParallel',true);
